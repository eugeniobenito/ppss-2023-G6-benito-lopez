...
public class Reserva {
    ...
    public void realizaReserva(String login, String password, String socio, String [] isbns) throws ReservaException {
        ...
        if(!compruebaPermisos(login, password, Usuario.BIBLIOTECARIO)) { // compruebaPermisos() es un DOC
            errores.add("ERROR de permisos");
        } else {
            IOperacionBO io = Factoria.create(); // Refactorizamos añadiendo una Factoría
            try {
                for(String isbn: isbns) {
                    try {
                        io.operacionReserva(socio, isbn); // operacionReserva() es un DOC
